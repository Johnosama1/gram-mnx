      ...
      GET: async ({ request }) => {
        ...
        if (newlyReached.length > 0) {
          ...
          const { data: claimedRows } = await db
            .from('gm_referral_milestone_credits')
            .upsert(
              newlyReached.map((m) => ({ telegram_id: user.id, milestone_id: m.id })),
              { onConflict: 'telegram_id,milestone_id', ignoreDuplicates: true },
            )
            .select('milestone_id');
          ...
          if (payout > 0) {
            await notifyUser(
              ...
              [
                ...
                '',
                `🎯 ${paid.map((m) => `${m.invite_count} referrals`).join(' + ')}`,
                `🎁 Reward: <b>${payout} coin</b>`,
              ].join('\n'),
            );
          }
