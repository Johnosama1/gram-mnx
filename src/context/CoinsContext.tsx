...
export function CoinsProvider({ children }: { children: React.ReactNode }) {
  ...
  // Re-sync the coin balance in the background after any balance-changing
  // action anywhere in the app (task, combo, swap, deposit, admin reset).
  useEffect(() => onDataChange((scopes) => {
    if (scopes.some((s) => s === 'balance' || s === 'admin' || s === 'wallet' || s === 'user')) {
