import { useEffect, useRef, useState } from 'react';
import { X } from 'lucide-react';
import { useTonConnectUI, useTonWallet } from '@tonconnect/ui-react';
import { useWallet } from '@/context/WalletContext';
import { useLanguage } from '@/context/LanguageContext';
import { motion, AnimatePresence } from 'framer-motion';
import { telegramApiPost, getInitData, API_BASE } from '@/lib/telegramApi';
import { toFriendlyAddress, shortFriendlyAddress } from '@/lib/tonAddress';

/** Shows first 4 chars + "..." + last 4 chars of the user-friendly TON address */
function shortAddress(addr: string): string {
  return shortFriendlyAddress(addr);
}


export default function WalletModal({ onClose }: { onClose: () => void }) {
  const [tonConnectUI] = useTonConnectUI();
  const tonWallet      = useTonWallet();
  const { connectWallet, walletAddress } = useWallet();
  const { t } = useLanguage();
  const [saveStatus, setSaveStatus] = useState<'idle' | 'saving' | 'ok' | 'err'>('idle');
  const [saveMsg,    setSaveMsg]    = useState('');

  const disconnectingRef = useRef(false);

  // When TON Connect gives us a new address, save it to the server (with uniqueness check)
  useEffect(() => {
    const raw = tonWallet?.account?.address;
    if (!raw || disconnectingRef.current) return;
    // Store/display the user-friendly address (UQ…) instead of the raw 0:… form.
    const addr = toFriendlyAddress(raw);



    const initData = getInitData();
    if (!initData) {
      // No Telegram context (desktop dev preview) — just save locally
      connectWallet(addr);
      return;
    }

    setSaveStatus('saving');
    fetch(`${API_BASE}/api/telegram/wallet`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ initData, address: addr }),
    })
      .then(async r => {
        if (!r.ok) {
          const data = await r.json().catch(() => ({})) as { message?: string };
          if (r.status === 409) {
            // Address taken by another account — disconnect immediately
            tonConnectUI.disconnect();
            setSaveStatus('err');
            setSaveMsg(data.message ?? t('wallet_address_taken'));
            return;
          }
          throw new Error(data.message ?? `HTTP ${r.status}`);
        }
        connectWallet(addr);
        setSaveStatus('ok');
        setSaveMsg('');
      })
      .catch(e => {
        // Server unavailable — save locally anyway, will sync later.
        // Wallet stays connected, so we don't surface a scary error.
        connectWallet(addr);
        console.warn('[wallet] save failed', e);
        setSaveStatus('idle');
        setSaveMsg('');
      });
  }, [tonWallet?.account?.address, connectWallet, tonConnectUI, t]);

  const handleConnect = () => {
    setSaveStatus('idle');
    setSaveMsg('');
    tonConnectUI.openModal();
  };

  const handleDisconnect = async () => {
    if (saveStatus === 'saving') return;
    setSaveStatus('saving');
    setSaveMsg('');
    // Block the auto-save effect from re-linking the address while we unlink.
    disconnectingRef.current = true;

    // Clear locally first so the UI always responds, even if the network or
    // the TON Connect session fails.
    connectWallet('');

    const initData = getInitData();
    if (initData) {
      await fetch(`${API_BASE}/api/telegram/wallet`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ initData }),
      }).catch(() => undefined);
    }

    // disconnect() throws when there is no active session — never block on it.
    try {
      await tonConnectUI.disconnect();
    } catch {
      /* already disconnected */
    }

    setSaveStatus('idle');
    onClose();
  };


  const connected     = Boolean(tonWallet?.account?.address || walletAddress);
  const displayAddress = tonWallet?.account?.address || walletAddress;
  const shortAddr      = displayAddress ? shortAddress(displayAddress) : '';

  return (
    <AnimatePresence>
      <div
        className="fixed inset-x-0 top-0 z-40 flex items-end justify-center"
        style={{ bottom: 'var(--nav-height)' }}
      >
        {/* Backdrop */}
        <motion.div
          className="absolute inset-0 bg-foreground/40"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          onClick={onClose}
        />

        {/* Sheet */}
        <motion.div
          className="relative w-full max-w-[640px] bg-white rounded-t-3xl p-6 pb-8 border-t border-violet-500/20"
          initial={{ y: '100%' }}
          animate={{ y: 0 }}
          exit={{ y: '100%' }}
          transition={{ type: 'spring', damping: 28, stiffness: 300 }}
        >
          {/* Handle bar */}
          <div className="absolute top-2 left-1/2 -translate-x-1/2 w-10 h-1 bg-secondary rounded-full" />

          {/* Header */}
          <div className="flex items-center justify-between mb-5 mt-2">
            <h2 className="text-xl font-black text-foreground">{t('wallet_title')}</h2>
            <button onClick={onClose} className="p-2 text-muted-foreground hover:text-foreground">
              <X className="w-5 h-5" />
            </button>
          </div>

          {connected ? (
            <div className="flex flex-col items-center gap-4 pb-2">
              <div className="text-success text-lg font-bold">{t('wallet_connected')}</div>

              {saveStatus === 'saving' && (
                <div className="text-xs text-primary animate-pulse">{t('wallet_saving')}</div>
              )}
              {saveStatus === 'err' && saveMsg && (
                <div className="text-xs text-red-400 text-center px-2 bg-red-500/10 rounded-xl p-2 border border-red-500/20">
                  ❌ {saveMsg}
                </div>
              )}
              {saveStatus === 'ok' && (
                <div className="text-xs text-success">{t('wallet_saved')}</div>
              )}

              <div className="bg-secondary rounded-xl px-6 py-2.5 border border-success/30">
                <span className="text-success font-mono text-base font-bold tracking-wider">{shortAddr}</span>
              </div>

              {tonWallet?.device?.appName && (
                <div className="text-xs text-muted-foreground/70">{tonWallet.device.appName}</div>
              )}

              <button
                onClick={handleDisconnect}
                className="w-full py-4 rounded-2xl border-2 border-red-500 bg-red-500/15 text-red-400 font-black text-base hover:bg-red-500/25 active:scale-95 transition-all"
              >
                {t('wallet_disconnect')}
              </button>
            </div>
          ) : (
            <div className="space-y-3">
              <p className="text-sm text-muted-foreground text-center mb-4">
                {t('wallet_choose')}
              </p>

              <button
                onClick={handleConnect}
                className="w-full flex items-center gap-4 p-4 rounded-2xl bg-secondary/60 border border-primary/30 hover:border-primary/60 transition-colors"
              >
                <div className="w-12 h-12 rounded-xl bg-primary/20 flex items-center justify-center">
                  <span className="text-2xl">💎</span>
                </div>
                <div className="text-left">
                  <div className="font-bold text-foreground">TON Connect</div>
                  <div className="text-xs text-muted-foreground">{t('wallet_ton_wallets')}</div>
                </div>
              </button>

              <p className="text-[10px] text-muted-foreground/50 text-center pt-2">
                {t('wallet_secure_note')}
              </p>
            </div>
          )}
        </motion.div>
      </div>
    </AnimatePresence>
  );
}
